...
async def _get_or_create_work_pool(client: PrefectClient, work_pool_name: str) -> WorkPool:
    """
    ...
    try:
        ...
        logger.info(f"Work Pool '{work_pool_name}' already exists.")
    except ObjectNotFound:
        # If the work pool does not exist, create a new one
        try:
            work_pool = await client.create_work_pool(WorkPoolCreate(name=work_pool_name, type='Process'))
            logger.info(f"Created Work Pool '{work_pool_name}'.")
        except Exception as e:
            # Log an error message if work pool creation fails and re-raise the exception
            logger.error(f"Failed to create work pool '{work_pool_name}': {e}")
            raise e
    
    return work_pool

# Function to get or create a work queue
async def _get_or_create_work_queue(client: PrefectClient, work_queue_name: str, work_pool_name: str) -> WorkQueue:
    """
    ...
    try:
        ...
        work_queue = await client.read_work_queue_by_name(name=work_queue_name, work_pool_name=work_pool_name)
        logger.info(f"Work Queue '{work_queue_name}' already exists.")
    except ObjectNotFound:
        # If the work queue does not exist, create a new one
        try:
            work_queue = await client.create_work_queue(name=work_queue_name, work_pool_name=work_pool_name)
            ...
            logger.error(f"Failed to create work queue '{work_queue_name}' in work pool '{work_pool_name}': {e}")
            raise e
    return work_queue
...
@task
async def create_work_pool_and_queue(work_pool_name: str, work_queue_name: str):
    """
    ...
    async with get_client() as client:
        ...
        work_pool = await _get_or_create_work_pool(client=client, work_pool_name=work_pool_name)
        
        # If the work pool exists, proceed to get or create the work queue
        if work_pool:
            _ = await _get_or_create_work_queue(client=client, work_queue_name=work_queue_name, work_pool_name=work_pool_name)