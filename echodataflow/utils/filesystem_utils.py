...
from echodataflow.models.deployment.storage_options import StorageOptions, StorageType
...
def _handle_storage_options_class(options: StorageOptions) -> Dict:
    if not options.anon:
        block = load_block(name=options.block_name, stype=options.block_type)
        return get_storage_options(block)
